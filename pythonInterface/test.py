...
class App(tk.Tk):
    ...
    def create_widgets(self):
        ...
        # Create the slider containers with new names
        motor_names = ["Claw", "Wrist 1", "Wrist 2", "Elbow", "Arm", "Base"]
        for i in range(1, 7):
            self.create_slider_container(self.scrollable_frame, i, motor_names[i-1])
    ...
    def create_slider_container(self, parent, index, motor_name):
        frame = ttk.Frame(parent, padding="10 10 10 10")
        frame.pack(pady=10, padx=20, fill='x')

        label = ttk.Label(frame, text=f"{motor_name} Control", style='TLabel')
        label.pack(pady=10)
        ...
        if index == 1:
            ...
            slider = ttk.Scale(frame, from_=80, to=350, orient='horizontal', style='TScale')
            slider.set(110)
        elif index == 3:
            slider = ttk.Scale(frame, from_=80, to=510, orient='horizontal', style='TScale')
            slider.set(250)
        elif index == 4:
            slider = ttk.Scale(frame, from_=250, to=400, orient='horizontal', style='TScale')
            slider.set(380)
        elif index == 5:
            slider = ttk.Scale(frame, from_=380, to=510, orient='horizontal', style='TScale')
            slider.set(430)
        elif index == 6:
            slider = ttk.Scale(frame, from_=100, to=520, orient='horizontal', style='TScale')
            slider.set(490)
